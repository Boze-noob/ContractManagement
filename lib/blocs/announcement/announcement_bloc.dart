...
class AnnouncementBloc extends Bloc<AnnouncementEvent, AnnouncementState> {
  ...
  static AnnouncementState initialState() => AnnouncementState(
      status: AnnouncementStateStatus.init, announcementsModels: List.empty());

  Future<void> _getAnnouncement(
      AnnouncementGetEvent event, Emitter<AnnouncementState> emit) async {
    emit(state.copyWith(status: AnnouncementStateStatus.loading));
    final result = await announcementRepo.getAnnouncements();
    if (result != null)
      emit(state.copyWith(
          status: AnnouncementStateStatus.loaded, announcementsModels: result));
    else
      emit(state.copyWith(
          status: AnnouncementStateStatus.error, message: 'Error happen'));
  }

  Future<void> _createAnnouncement(
      AnnouncementCreateEvent event, Emitter<AnnouncementState> emit) async {
    if (event.orderModel.receiverId == null) {
      emit(state.copyWith(
          status: AnnouncementStateStatus.error,
          message: 'Receiver id is not defined'));
    } else {
      ...
      final result =
          await announcementRepo.createAnnouncement(announcementModel);
      if (result)
        emit(state.copyWith(
            status: AnnouncementStateStatus.created,
  ...
  Future<void> _deleteAnnouncement(
      AnnouncementDeleteEvent event, Emitter<AnnouncementState> emit) async {
    final result =
        await announcementRepo.deleteAnnouncement(event.announcementId);
    if (result == null)
      emit(state.copyWith(
          status: AnnouncementStateStatus.deleted,
          message: 'Deleted successfully'));
    else
      emit(state.copyWith(
          status: AnnouncementStateStatus.error, message: result));
  }

  Future<void> _sendAnnouncement(
      AnnouncementSendEvent event, Emitter<AnnouncementState> emit) async {
    final result = await announcementRepo.sendAnnouncement(
        event.announcementId, event.receiverId);
    if (result == null)
      emit(state.copyWith(
          status: AnnouncementStateStatus.sent, message: 'Announcement sent'));
    else
      emit(state.copyWith(
          status: AnnouncementStateStatus.error, message: result));
  }

  Future<void> _declineAnnouncement(
      AnnouncementDeclineEvent event, Emitter<AnnouncementState> emit) async {
