...
class OverviewPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: BlocProvider(
        create: (context) => RevenueBloc(revenueRepo: context.serviceProvider.revenueRepo)..add(RevenueLoadEvent()),
                    ...
                    builder: (context, state) {
                      return BlocBuilder<RevenueBloc, RevenueState>(
                        builder: (context, revenueState) {
                          if (revenueState.status == RevenueStateStatus.loading || revenueState.status == RevenueStateStatus.error)
                            return Center(
                              child: Loader(
                                width: 100,
                                height: 100,
                            ...
                            return ListView(
                              children: [
                                ...
                                if (!ResponsiveWidget.isSmallScreen(context)) RevenueSectionLarge(revenueState.revenueModel) else RevenueSectionSmall(revenueState.revenueModel),
